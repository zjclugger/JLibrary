...
/**
 * 票据报销统计<br>
 * Created by King.Zi on 2020/02/07.<br>
 * Copyright (c) 2020 zjclugger.com
 */
public class BillReimbursementFragment extends BaseFragment {
    ...
    private List<ERPListItem<String>> mDepartsResultList = new ArrayList<>();
    ...
    public void initViews() {
        initDetailTitleViews(getString(R.string.bill_reimbursement), null);
        statisticalTitleView.setLabelText(R.string.bill_reimbursement);
        ...
        mClassPickerView = new OptionsPickerView(mContext, new OnOptionsSelectListener() {
            @Override
            public void onOptionsSelect(int options1, int options2, int options3, View v) {
                classView.setValueText(mDepartsResultList.get(options1).getText());
                mQueryCondition.put(Constants.QueryParameter.COMPANY_ID,
                        mDepartsResultList.get(options1).getValue());
                getDataFromServer(true);
            }
        });
    ...
    private void initObjectListView() {
        //从接口获取对象列表
        showWaiting();
        mViewModel.getOrganizationDeparts().observe(this,
                response -> {
                    if (mDepartsResultList != null) {
                        mDepartsResultList.clear();
                    }

                    if (ApiResponseUtils.isSuccess(response, getString(R.string.error_get_depart_failed))) {
                        JsonArray jsonArray = response.body.getResult().getAsJsonArray();
                        mDepartsResultList.add(0, new ERPListItem(getString(R.string.all), ""));

                        if (null != jsonArray) {
                            for (JsonElement org : jsonArray) {
                                mDepartsResultList.add(new ERPListItem(org.getAsJsonObject().get(
                                        "orgName").getAsString(),
                                        org.getAsJsonObject().get("id").getAsString()));
                            }
                        }

                        mClassPickerView.bindData(mDepartsResultList);
                        classView.setValueText(mDepartsResultList.get(0).getText());
                        mQueryCondition.put(Constants.QueryParameter.COMPANY_ID, "");
                    }
    ...
    private void getDataFromServer(boolean isRefresh) {
        ...
        mViewModel.getReportForm(mQueryCondition).observe(this,
                new Observer<ApiResponse<BaseWrapperEntity<ReimbursementResult>>>() {
                    @Override
                    public void onChanged(ApiResponse<BaseWrapperEntity<ReimbursementResult>> response) {
                        closeProgressDialog();
                        //非分页查询先清空数据
                        if (mDataList != null) {
                            mDataList.clear();
